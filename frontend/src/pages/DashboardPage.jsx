...
function HeroStat({ label, value, icon, gradient, glow, badge }) {
  return (
    <div className={`rounded-2xl p-5 relative overflow-hidden ${gradient} shadow-md`}>
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3">
          ...
          {badge && (
            <span className="text-white/80 text-xs font-semibold bg-white/20 px-2.5 py-1 rounded-full">
              {badge}
            </span>
          )}
        </div>
...
function QuickAction({ icon, label, desc, color, onClick }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-gray-100 hover:border-teal-200 hover:shadow-md transition-all duration-200 text-left group w-full"
    >
      <div className={`w-12 h-12 rounded-xl ${color} flex items-center justify-center text-xl flex-shrink-0 group-hover:scale-110 transition-transform duration-200`}>
        {icon}
      </div>
      <div>
      ...
      <svg className="w-4 h-4 text-gray-300 group-hover:text-teal-500 ml-auto transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fade-in">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {stats.districts && stats.districts.length > 0 ? (
              <div className="space-y-3">
                {stats.districts.slice(0, 8).map((d, idx) => {
                  const pct = total > 0 ? Math.round((d.count / total) * 100) : 0;
                  const barColors = ['bg-teal-500', 'bg-cyan-500', 'bg-blue-500', 'bg-indigo-500'];
                  const color = barColors[idx % barColors.length];
                  return (
                    <div key={d.name} className="flex items-center gap-3">
                      <span className="text-xs font-bold text-gray-400 w-5 text-right">{idx + 1}</span>
                      <span className="text-sm font-medium text-gray-700 w-36 truncate">{d.name}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
            <h3 className="font-bold text-gray-900">Risk Breakdown</h3>
            {total > 0 ? (
              <div className="space-y-3">
                {[
            ...
            {topDistrict && (
              <div className="mt-4 bg-teal-50 border border-teal-100 rounded-xl p-3">
                ...
                <p className="font-bold text-teal-800 mt-0.5">{topDistrict.name}</p>
          ...
          {/* Health Tip */}
          <div className="rounded-2xl bg-gradient-to-br from-teal-600 to-cyan-600 p-6 text-white shadow-md relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white text-sm leading-relaxed font-medium">{tip.tip}</p>
